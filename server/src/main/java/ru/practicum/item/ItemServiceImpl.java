...
public class ItemServiceImpl implements ItemService {
    ...
    @Override
    public ItemDto addItem(ItemDto itemDto, long userId, Long requestId) {
        log.info("Добавление itemDto: {} для userId {}", itemDto, userId);
        User user = userService.findUserById(userId);

        // Используем requestId из itemDto
        Long actualRequestId = itemDto.getRequestId();
        ItemRequest itemRequest = null;
        if (actualRequestId != null) {
            itemRequest = requestRepository.findById(actualRequestId)
                    .orElseThrow(() -> new IllegalArgumentException("Запрос с указанным ID не найден."));
        }
